    ...
    <div class="row " style="background-color: #edf0ed;" >
       ...
       <?php   include('partials/sidebar.php'); ?> <!---sidebar --->
    	...
    	<div class="col-md-12  m-5 main ">
            <h3 class="text-monospace text-center text-bold">Your All Assignment</h3>
...
if ($result->num_rows > 0) {
    echo "<table class='table'>
        <thead >
            <tr>
                <th scope='col' class='bg-warning'>ID</th>
                <th scope='col' class='bg-warning'>Teacher Name</th>
                <th scope='col' class='bg-warning'>Course Code</th>
                <th scope='col' class='bg-warning'>Assignment No</th>
                <th scope='col' class='bg-warning'>Instruction</th>
                <th scope='col' class='bg-warning'>PDF</th>
                <th scope='col' class='bg-warning'>Submission Link</th>
                <th scope='col' class='bg-warning'>Section</th>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td data-label='Assignment No'>" . $row["assignment_no"] . "</td>";
        echo "<td data-label='Instruction'>" . htmlspecialchars($instruction) . "</td>";
        echo "<td data-label='PDF'><a href='" ."uploads/assignments/". $row["pdf"] . "' target='_blank'>Download PDF</a></td>";
        echo "<td data-label='Submission Link'><a href='" . $row["assign_sub_link"] . "' target='_blank'>Submission Link</a></td>";
        echo "<td data-label='Section'>" . $row["section"] . "</td>";
        echo "<td data-label='Section'>
              <a href='edit_assignment.php?id=$id' class='btn btn-success'>&#9998;</a>
              <a href='delete_assignment.php?id=$id' class='btn btn-danger'><i class='fas fa-trash'></i>
              </a>
            </td>";
        echo "</tr>";
    }
